$(function(){
  ...
  const App2 = {
     ...
     * ====================== */
    initAccordion(){
      if (!$('.intro-savista').length) return;
     ...
     * ====================== */
    initPhaseHover(){
      if (!$('.phase-row').length) return;
      $(".phase-row").each(function(){
        const $row   = $(this);
        const $shape1 = $row.find(".phase-shape1");
        const $shape2 = $row.find(".phase-shape2-wrap");

        $shape1.hover(
          () => { $shape1.addClass("shape1-hover"); $shape2.addClass("shape2-hover"); },
          () => { $shape1.removeClass("shape1-hover"); $shape2.removeClass("shape2-hover"); }
        );
        $shape2.hover(
          () => { $shape2.addClass("shape2-hover"); $shape1.addClass("shape1-hover"); },
          () => { $shape2.removeClass("shape2-hover"); $shape1.removeClass("shape1-hover"); }
        );
      });
    },
     ...
     * ====================== */
    initStudyMethod(){
      if (!$('.ds-pencil').length) return;
      $(".ds-pencil").hover(
        function(){ $(this).addClass("pencil-hover"); },
        function(){ $(this).removeClass("pencil-hover"); }
      );
    },
     ...
     * ====================== */
    initShapeHover(){
      if (!$('.ds-head-shape, .ds-hank-shape, .ds-tail-shape').length) return;
    ...
    initKnowledgeShapes(){
      ...
      if (!rootEl) return; // tránh lỗi getComputedStyle nếu không có element
      const cssVar = getComputedStyle(rootEl);

      const sizeShape1     = parseFloat(cssVar.getPropertyValue('--s1-size'));
      ...
      const shape2Content = [
        "Định hướng","Quản lý thời gian","Rèn luyện tư duy","Giải quyết vấn đề",
        "Khơi dậy ý tưởng","Theo dõi tiến bộ","Học nhóm","Đạt mục tiêu"
      ];
      const shape1Paths = [
      ...
      const darkenColor = (hex, percent)=>{
        let hexC = hex.replace('#','');
        if(hexC.length===3) hexC = hexC.split('').map(ch=>ch+ch).join('');
        let r=parseInt(hexC.substr(0,2),16),
            g=parseInt(hexC.substr(2,2),16),
            b=parseInt(hexC.substr(4,2),16);
      ...
      const getRotateBox = idx=>{
        if(idx>=1 && idx<=4) return 120+(idx-1)*10;
        const mirror={5:2,6:1,7:4,8:3};
        return 120+(mirror[idx]-1)*10;
      };
      const buildPathShape2 = (w,h,side)=>{
        const r=h/2, s=0.45;
        return side==='left'
          ? `M${w},${h} H${r} C${r*s},${h} 0,${h-r*s} 0,${h/2} C0,${r*s} ${r*s},0 ${r},0 H${w} V${h} Z`
          : `M0,0 H${w-r} C${w-r*s},0 ${w},${r*s} ${w},${h/2} C${w},${h-r*s} ${w-r*s},${h} ${w-r},${h} H0 Z`;
      };

      // render shape1
      $('.km-item').each(function(i){
        const idx=i+1, colors=colorShape2[i], border=darkenColor(colors[0],25);
        const ang=-135+45*i, rotateBox=getRotateBox(idx), gradId=`grad${idx}`;
        $(this).html(`
          <div class="km-orbit" style="transform:rotate(${ang}deg);">
            <div class="km-s1-box" data-idx="${idx}"
              ...
              <i class="km-s1-ico fa-solid ${faIcons[i]}" style="--ico-g1:${colorIcons[i][0]}; --ico-g2:${colorIcons[i][1]};"></i>
            </div>
          </div>`);
      });
      ...
      const items=[];
      $('.km-item').each(function(i){
        const idx=i+1, side=sideMap[idx], s1=$(this).find('.km-s1-box')[0];
      ...
      items.forEach(item=>{
        const {idx,side}=item;
        const extraW=[1,2,5,6].includes(idx)?80:0;
        const w=baseWidthShape2+extraW, h=baseHeightShape2;
        let posAttr,posVal;
        if([2,3,4,5].includes(idx)){ posAttr="right"; posVal=[3,4].includes(idx)?95:97; }
        else { posAttr="left"; posVal=[7,8].includes(idx)?95:97; }
        const row=rowIndexOf(idx);
        const top=centerY+(row-1.5)*rowGap-h/2;
        const gradId=`gradS2_${idx}`, border=darkenColor(colorShape2[idx-1][0],35);
        const numSize=h*0.81;
        ...
        $('.km-stage').append(`
          <div class="km-s2-box ${side}" data-idx="${idx}"
            ...
            <svg class="km-s2-svg" viewBox="0 0 ${w} ${h}">
              ...
              <path d="${buildPathShape2(w,h,side)}" fill="url(#${gradId})" stroke="${border}" stroke-width="2"/>
            </svg>
            <div class="km-s2-num ${side}" style="width:${numSize}px;height:${numSize}px;">
              <span class="number_shape">${String(idx).padStart(2,'0')}</span>
            </div>
            <div class="km-s2-text" style="background:${textGradients[idx-1]}; -webkit-background-clip:text; -webkit-text-fill-color:transparent; text-shadow:${textShadows[idx-1]};">
              ${shape2Content[idx-1]}
            </div>
          </div>`);
      });

      // hover link shape1 & shape2
      $('.km-stage')
        .on('mouseenter','.km-s1-box',function(){
        ...
        .on('mouseenter','.km-s2-box',function(){
          const idx=$(this).data('idx');
          $(this).addClass('active');
    ...
    init(){
      this.initAccordion();
      this.initPhaseHover();
      this.initStudyMethod();
      this.initRowHover();
      this.initShapeHover();
      this.initKnowledgeShapes();
    }
  };
